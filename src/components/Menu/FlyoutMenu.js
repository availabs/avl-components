...
const FlyoutMenu = ({open,items,bottomItems}) => {
	return (
		<div className={`absolute  inset-x-0 transform shadow-lg ${open ? '' : 'hidden'}`}>
		  <div className="bg-white">
		    <div className="max-w-7xl mx-auto grid gap-y-6 px-4 py-6 sm:grid-cols-2 sm:gap-8 sm:px-6 sm:py-8 lg:grid-cols-4 lg:px-8 lg:py-12 xl:py-16">
		      {items.map(item => 
			      <a href={`${item.href}`} className="-m-3 p-3 flex flex-col justify-between rounded-lg hover:bg-gray-50 transition ease-in-out duration-150">
		  ...
		  <div className="bg-gray-50">
		    <div className="max-w-7xl mx-auto space-y-6 px-4 py-1 sm:flex sm:space-y-0 sm:space-x-10 sm:px-6 lg:px-8">
		      {bottomItems.map(item => <div className='flow-root'>{item}</div>)}
		    </div>
		  </div>
		</div>
	)
